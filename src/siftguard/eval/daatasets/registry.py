"""Dataset registry for SIFTGuard ablation experiments.

Single source of truth for case_id → ground truth path + evidence path + briefing.
Does NOT duplicate ground truth JSON content — reads it at runtime via load_ground_truth().
"""
...
def available_datasets() -> list[str]:
    """Return case_ids where both image and ground truth exist on disk."""
    return [
        cid for cid, meta in _REGISTRY.items()
        if meta.evidence_available and meta.ground_truth_available
    ]