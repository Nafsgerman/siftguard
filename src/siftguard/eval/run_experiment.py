...
async def _dispatch(
    ...
    if agent_id == "siftguard-langgraph":
        ...
        result = await run_case_langgraph(case_id=case_id, evidence_files=evidence,
                                          briefing=briefing, audit_db=audit_db, on_event=None,
        ...
        result = await run_case_openai_fc(case_id=case_id, evidence_files=evidence,
                                          briefing=briefing, audit_db=audit_db, on_event=None,
                                          system_prompt_prefix=preamble)
    elif agent_id == "siftguard-gemini":
        from siftguard.orchestrators.gemini_adapter import run_case_gemini
        result = await run_case_gemini(case_id=case_id, evidence_files=evidence,
                                       briefing=briefing, audit_db=audit_db, on_event=None,
                                       system_prompt_prefix=preamble)
        ...
        result = await run_case(case_id=case_id, evidence_files=evidence,
                                briefing=briefing, audit_db=audit_db, on_event=None,
                                system_prompt_prefix=preamble)
    else:
...
def _write_raw_result(agent_id: str, case_id: str, payload: Any, ts: str) -> Path:
    ...
    body = payload if isinstance(payload, (dict, list, str, int, float)) else \
        (payload.report if hasattr(payload, "report") else {"raw": str(payload)})
...
async def _run(agent_id: str, case_id: str, gt_version: str) -> int:
    ...
    if run_id:
        try:
            score = score_run(case_id=case_id, agent_id=agent_id,
                              gt_version=gt_version, audit_db_path=audit_db, run_id=run_id)
        except FileNotFoundError as exc:
            log.warning("GT file not found (%s) -- skipping score", exc)
    if score:
        log.info("f1_applicable=%s tp=%d fp=%d fn=%d applicable=%d total=%d",
                 f"{score.f1_applicable:.3f}" if score.f1_applicable is not None else "None",
                 score.tp, score.fp, score.fn_applicable,
                 score.applicable_count, score.total_count)
        data_path = update_panel_7(
            case_id=case_id, agent_id=agent_id, f1=score.f1_applicable,
            gt_version=gt_version, applicable_count=score.applicable_count,
...
def _parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    ...
    p.add_argument("--gt-version", default="1.1.0", help="Ground truth schema version (default 1.1.0)")
