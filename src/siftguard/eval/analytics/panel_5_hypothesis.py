...
from siftguard.eval.analytics.style import (
    apply_style, add_claim, placeholder, BLUE, GREEN, RED, YELLOW, GRAY
)
...
EVENT_MARKERS = {
    "formed": "o", "updated": "s", "confirmed": "*", "abandoned": "X"
}
...
def render(ax: matplotlib.axes.Axes, case_id: str = "TEST-001") -> dict:
    ...
    if not db_path.exists():
        placeholder(ax, "Panel 5 — Hypothesis Evolution",
    ...
    if not events:
        placeholder(
            ax, "Panel 5 — Hypothesis Evolution",
    ...
    for event in events:
        ...
        ax.scatter(x, y + conf * 0.8, c=color, marker=marker,
                   s=80, zorder=3, alpha=0.85)

    for etype, color in EVENT_COLORS.items():
        ax.scatter([], [], c=color, marker=EVENT_MARKERS[etype],
                   label=etype, s=60)

    ax.set_yticks(range(len(hyp_ids)))
    ax.set_yticklabels(
        [hid[:20] for hid in hyp_ids], fontsize=7
    )
