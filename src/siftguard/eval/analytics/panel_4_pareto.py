...
from siftguard.eval.analytics.style import (
    apply_style, add_claim, placeholder, BLUE, RED, YELLOW, GRAY, LGRAY
)
...
def _is_pareto(costs: list, f1s: list) -> list[bool]:
    ...
    for i in range(n):
        for j in range(n):
            if i == j:
                continue
            if costs[j] <= costs[i] and f1s[j] >= f1s[i] and (
                costs[j] < costs[i] or f1s[j] > f1s[i]
            ):
                on_frontier[i] = False
                break
    return on_frontier
...
def render(ax: matplotlib.axes.Axes, case_id: str = "TEST-001") -> dict:
    ...
    if not db_path.exists():
        placeholder(ax, "Panel 4 — Cost-Accuracy Pareto",
    ...
    for run in runs:
        ...
        findings = _findings_from_json(
            json.loads(last.get("findings_json") or "[]")
        )
    ...
    for i, (p, frontier) in enumerate(zip(points, on_f)):
        ...
        ax.scatter(p["cost"], p["f1"], c=color, marker=marker,
                   s=size, zorder=3, alpha=0.85)
        ax.annotate(
            p["label"][:20],
            (p["cost"], p["f1"]),
            textcoords="offset points",
            xytext=(5, 4), fontsize=6, color=GRAY,
        )
    ...
    if len(frontier_pts) > 1:
        ...
        ax.step(fx, fy, where="post", color=BLUE, linewidth=1.5,
