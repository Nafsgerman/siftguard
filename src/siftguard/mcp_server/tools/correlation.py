...
from siftguard.mcp_server.safe_exec import safe_exec
...
async def correlate_findings(*, memory_image: str) -> ForensicResult:
    ...
    pslist_pids = {int(r.get("PID") or 0) for r in pslist_rows if r.get("PID")}
    psscan_pids = {int(r.get("PID") or 0) for r in psscan_rows if r.get("PID")}
