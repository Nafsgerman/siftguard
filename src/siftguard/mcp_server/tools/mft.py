    ...
    *,
    ...
    if err:
        return ForensicResult(tool="analyze_mft", outcome=ToolOutcome.FAIL,
                              summary=err, duration_ms=duration_ms, error=err)
    ...
    try:
        ...
        for r in raw:
            ...
            entries.append({
                "record_number": record_num, "filename": filename, "in_use": in_use,
                "file_type": file_type, "created": created, "modified": modified,
                "timestomp_suspected": timestomp, "raw": r,
            })
    except Exception as e:
        return ForensicResult(tool="analyze_mft", outcome=ToolOutcome.FAIL,
                              summary=f"JSON parse failed: {e}",
                              raw_excerpt=stdout[:1500], duration_ms=duration_ms, error=str(e))
    ...
    return ForensicResult(
        ...
        findings=entries[:200], raw_excerpt=stdout[:1500],
        evidence_refs=[memory_image], duration_ms=duration_ms,
    )
...
async def scan_mft_ads(*, memory_image: str) -> ForensicResult:
    ...
    return ForensicResult(
        ...
        findings=findings[:100], raw_excerpt=stdout[:1500],
        evidence_refs=[memory_image], duration_ms=duration_ms,
    )
