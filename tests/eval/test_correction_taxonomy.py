...
from siftguard.models.correction_taxonomy import SelfCorrectionType, classify_correction
...
@pytest.mark.parametrize("message,expected", [
    ...
    ("Verdict revised after reviewing new memory artifact", SelfCorrectionType.VERDICT_REVISION),
    ...
    ("Retracting finding — unverifiable in memory image",   SelfCorrectionType.HALLUCINATION_RETRACT),
...
def _make_mock_db(rows: list[dict]) -> str:
    ...
    for r in rows:
        conn.execute(
            ...
            (r["id"], r["case_id"], r.get("agent_iteration", 1), r.get("tool_name", ""), r.get("correction_event"))
        )
    conn.commit()
    conn.close()
    return tmp
...
def test_breakdown_shape():
    db = _make_mock_db([
        {"id": 1, "case_id": "CASE-001", "agent_iteration": 1, "correction_event": "Pydantic validation error"},
        {"id": 2, "case_id": "CASE-001", "agent_iteration": 1, "correction_event": "Volatility tool failed"},
        {"id": 3, "case_id": "CASE-001", "agent_iteration": 2, "correction_event": "Verdict revised"},
        {"id": 4, "case_id": "CASE-001", "agent_iteration": 2, "correction_event": "nothing matches"},
        {"id": 5, "case_id": "CASE-001", "agent_iteration": 2, "correction_event": None},  # excluded
    ])
    try:
...
def test_filters_by_case_id():
    db = _make_mock_db([
        {"id": 1, "case_id": "CASE-001", "agent_iteration": 1, "correction_event": "tool failed"},
        {"id": 2, "case_id": "CASE-999", "agent_iteration": 1, "correction_event": "tool failed"},
    ])
    try:
        result = get_correction_breakdown(db, "CASE-001")
