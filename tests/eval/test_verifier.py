...
def _make_db(tool_outputs: list) -> str:
    ...
    con.execute("CREATE TABLE auditentry (id INTEGER PRIMARY KEY AUTOINCREMENT, run_id TEXT, tool_output TEXT, correction_event TEXT)")
...
def test_exact_match_verified():
    ...
    result = verify_finding({"id":"F-001","value":"172.16.4.10:8080","type":"network","description":"C2 channel"}, db, "RUN-001")
    os.unlink(db)
...
def test_substring_match_partial_value():
    ...
    result = verify_finding({"id":"F-002","value":"5682","type":"network","description":"backdoor port"}, db, "RUN-001")
    os.unlink(db)
    assert result.status == VerificationStatus.VERIFIED
    assert result.method == VerificationMethod.SUBSTRING_MATCH
...
def test_description_keyword_fallback():
    ...
    result = verify_finding({"id":"F-003","value":"HKLM\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run\\Backdoor","type":"registry","description":"persistence registry key for malware startup"}, db, "RUN-001")
    os.unlink(db)
    assert result.status == VerificationStatus.VERIFIED
...
def test_paraphrase_miss_refuted():
    ...
    result = verify_finding({"id":"F-004","value":"33001","type":"network","description":"second backdoor port"}, db, "RUN-001")
    os.unlink(db)
...
def test_empty_audit_corpus():
    db = _make_db([])
    result = verify_finding({"id":"F-005","value":"172.16.4.10","type":"network","description":"C2 IP"}, db, "RUN-001")
    os.unlink(db)
...
def test_empty_finding_value():
    ...
    result = verify_finding({"id":"F-006","value":"","type":"network","description":"missing value"}, db, "RUN-001")
    os.unlink(db)
    assert result.status == VerificationStatus.UNVERIFIABLE

def test_whitelisted_process_verified():
    db = _make_db([])
    result = verify_finding({"id":"F-007","value":"svchost.exe","type":"process","description":"windows service host"}, db, "RUN-001")
    os.unlink(db)
    assert result.status == VerificationStatus.VERIFIED
...
def test_tool_rerun_path_refuted(monkeypatch):
    ...
    monkeypatch.setattr("siftguard.eval.verifier._tool_rerun_verify", lambda v, t: None)
    result = verify_finding({"id":"F-008","value":"172.16.4.10","type":"network","description":"C2 IP"}, db, "RUN-001", enable_tool_rerun=True)
    os.unlink(db)
    assert result.status == VerificationStatus.REFUTED
...
def test_tool_rerun_path_verified(monkeypatch):
    db = _make_db(["unrelated output only"])
    monkeypatch.setattr("siftguard.eval.verifier._tool_rerun_verify", lambda v, t: "TCPv4 ESTABLISHED 172.16.4.10:8080 pid=666")
    result = verify_finding({"id":"F-009","value":"172.16.4.10","type":"network","description":"C2 IP"}, db, "RUN-001", enable_tool_rerun=True)
    os.unlink(db)
    assert result.status == VerificationStatus.VERIFIED
    assert result.method == VerificationMethod.TOOL_RERUN
